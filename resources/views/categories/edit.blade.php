...
@section('content')
        <h1 class="text-2xl text-blue-900 font-bold mb-4">Modifica Categoria</h1>
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST" class="bg-white p-6 rounded shadow-md">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="name" name="name" value="{{ $category->name }}" class="p-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                @error('name')
